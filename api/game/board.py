...
class Point:
    ...
    def is_pinned(self, color: PlayerColor) -> bool:
        """Check if this point has a pinned checker (single checker with opponent on top)."""
        # In pinning variants, a single checker is pinned if opponent has pieces on the same point
        if self.get_pieces(color) == 1:
            opponent = color.opposite()
            return self.get_pieces(opponent) > 0
        return False
