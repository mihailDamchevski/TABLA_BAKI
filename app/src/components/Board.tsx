...
const Board: React.FC<BoardProps> = ({
  ...
  const renderBorneOffArea = (
    ...
    return (
      <div
        className={`w-12 min-w-[3rem] sm:w-[60px] sm:min-w-[60px] flex flex-col items-center justify-center py-3 px-2 bg-gradient-to-b from-[#5C3A21] via-[#654321] to-[#5C3A21] border-l-[3px] border-r-[3px] border-[#3D2416] shadow-[inset_2px_0_6px_rgba(0,0,0,0.4),inset_-2px_0_6px_rgba(0,0,0,0.4)] relative z-0 ${side === "left" ? "border-r-[4px] rounded-l-xl" : "border-l-[4px] rounded-r-xl"}`}
      >
        <div className="text-[10px] font-extrabold text-white/70 drop-shadow-[1px_1px_2px_rgba(0,0,0,0.8)] tracking-[1.5px] mb-2 font-['Righteous','Bebas_Neue',cursive] [writing-mode:vertical-rl] [text-orientation:mixed] rotate-180">
        ...
        <div className="flex-1 flex flex-col items-center justify-start gap-[3px] w-full p-1 relative min-h-0 overflow-y-auto overflow-x-hidden [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-black/20 [&::-webkit-scrollbar-thumb]:bg-white/20 [&::-webkit-scrollbar-thumb]:rounded-sm">
          ...
          {count > 15 && (
            <div className="absolute bottom-1 left-1/2 -translate-x-1/2 bg-[rgba(255,215,0,0.9)] text-black text-[10px] font-extrabold py-0.5 px-1.5 rounded-[10px] shadow-[0_2px_4px_rgba(0,0,0,0.3)] z-10">
              {count}
            </div>
          )}
        </div>
  ...
  const renderPoint = (pointNumber: number, isTop: boolean) => {
    ...
    return (
      ...
      >
        ...
        <div
          className={`absolute left-1/2 -translate-x-1/2 text-[17px] font-extrabold font-['Righteous','Bebas_Neue',cursive] text-white drop-shadow-[2px_2px_4px_rgba(0,0,0,0.9),0_0_10px_rgba(0,0,0,0.6),0_1px_2px_rgba(0,0,0,0.8)] z-[15] tracking-[1.5px] bg-gradient-to-b from-white/20 to-white/5 py-0.5 px-1.5 rounded backdrop-blur-sm ${isTop ? "top-1" : "bottom-1"}`}
        >
          {pointNumber}
        </div>
        <div
            ...
            <div className="flex flex-col items-center relative w-full gap-0.5 z-[1001] translate-z-0">
              {Array.from({
                ...
                return (
                  <div
                    key={idx}
                    className="w-[40px] h-[40px] sm:w-[50px] sm:h-[50px] rounded-full flex items-center justify-center relative flex-shrink-0 z-[1002] translate-z-0 transition-all hover:scale-105 bg-gradient-to-br from-white via-[#F5F5F5] to-[#E8E8E8] border-[3px] border-[#D0D0D0] shadow-[0_4px_8px_rgba(0,0,0,0.25),0_2px_4px_rgba(0,0,0,0.15),inset_0_2px_4px_rgba(255,255,255,0.9),inset_0_-2px_4px_rgba(0,0,0,0.1)] before:content-[''] before:absolute before:top-[20%] before:left-[20%] before:w-[30%] before:h-[30%] before:rounded-full before:bg-white/60 before:blur-sm"
            ...
            <div className="flex flex-col items-center relative w-full gap-0.5 z-[1001] translate-z-0">
              {Array.from({
                ...
                return (
                  <div
                    key={idx}
                    className="w-[40px] h-[40px] sm:w-[50px] sm:h-[50px] rounded-full flex items-center justify-center relative flex-shrink-0 z-[1002] translate-z-0 transition-all hover:scale-105 bg-gradient-to-br from-[#1a1a1a] via-black to-[#0a0a0a] border-[3px] border-[#2a2a2a] shadow-[0_4px_8px_rgba(0,0,0,0.5),0_2px_4px_rgba(0,0,0,0.3),inset_0_2px_4px_rgba(255,255,255,0.15),inset_0_-2px_4px_rgba(0,0,0,0.5)] before:content-[''] before:absolute before:top-[20%] before:left-[20%] before:w-[30%] before:h-[30%] before:rounded-full before:bg-white/10 before:blur-sm"
                    style={{ opacity: shouldHide ? 0 : 1 }}
                  >
                    {idx === Math.min(blackPieces, MAX_VISIBLE_PIECES) - 1 &&
      ...
      >
        ...
        <div className="flex flex-row h-full w-full gap-0 items-stretch relative z-[2]">
          ...
          <div className="flex flex-row h-full gap-0.5 z-0 relative flex-1">
            ...
            <div
              className={`w-16 min-w-[4rem] sm:w-20 sm:min-w-20 bg-gradient-to-b from-[#5C3A21] via-[#654321] to-[#5C3A21] border-l-[4px] border-r-[4px] border-[#3D2416] flex-shrink-0 shadow-[inset_3px_0_8px_rgba(0,0,0,0.4),inset_-3px_0_8px_rgba(0,0,0,0.4),inset_0_2px_4px_rgba(255,255,255,0.1)] flex items-center justify-center z-0 relative cursor-pointer select-none transition-all ${
                selectedPoint === 0
            ...
            >
              <div className="flex flex-col items-center gap-2.5 p-2.5 w-full">
                <div className="font-extrabold text-white/90 text-[13px] drop-shadow-[1px_1px_3px_rgba(0,0,0,0.8),0_0_6px_rgba(0,0,0,0.5)] tracking-[2px] font-['Righteous','Bebas_Neue',cursive]">
                  Bar
                </div>
